...
import { ListMusic, ArrowLeft, Send, Play, FileText } from 'lucide-react';

interface LoginM3uProps {
  fieldIndex: number; // 0: URL input, 1: Raw textarea, 2: Submit, 3: Back, 4: Demo Mode shortcut
  urlValue: string;
...
export default function LoginM3u({
  fieldIndex,
  urlValue,
  onChangeUrl,
  rawValue,
  ...
  // Focus elements dynamically based on remote key index
  useEffect(() => {
    if (isLoading) return;
    
    if (fieldIndex === 0) urlRef.current?.focus();
    else if (fieldIndex === 1) rawRef.current?.focus();
    else if (fieldIndex === 2) submitRef.current?.focus();
    else if (fieldIndex === 3) backRef.current?.focus();
    else if (fieldIndex === 4) demoRef.current?.focus();
  ...
  return (
    <div className="min-h-screen w-full bg-[#050505] flex flex-col items-center justify-center p-6 select-none relative">
      ...
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-[#0066FF]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-[#0066FF]/5 rounded-full blur-3xl" />
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-extrabold text-white tracking-tight uppercase">Lista M3U</h1>
          <p className="text-white/40 text-xs font-mono tracking-wider mt-1 uppercase">CARGAR VÍA URL O PEGAR CONTENIDO</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest block font-mono">URL de la Lista</label>
            <div className="relative">
              ...
              <input
                ref={urlRef}
                type="text"
                placeholder="Ej. http://server.xyz/list.m3u"
                value={urlValue}
          ...
          {/* Raw Textarea option (very helpful for sandbox testing) */}
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest block font-mono">Pegar contenido M3U (Opcional)</label>
            <div className="relative">
              ...
              <textarea
                ref={rawRef}
                placeholder={`Escribe o pega tu archivo M3U aquí...\nEjemplo:\n#EXTM3U\n#EXTINF:-1 tvg-logo="https://logo.png" group-title="Mi Grupo",Canal 1\nhttp://stream-url.m3u8`}
                value={rawValue}
                onChange={(e) => onChangeRaw(e.target.value)}
                disabled={isLoading}
                rows={4}
                className={`w-full bg-[#141414] text-white pl-12 pr-4 py-3 rounded-xl border outline-none text-sm font-mono resize-none transition-all duration-200 placeholder-white/25 ${
                  fieldIndex === 1
                    ? 'border-[#0066FF] bg-[#141414]/90 shadow-[0_0_15px_rgba(0,102,255,0.2)] scale-[1.02]'
                    : 'border-white/5 focus:border-white/10'
                }`}
        ...
        <div className="mt-6 pt-6 border-t border-white/5 flex flex-col items-center">
          <p className="text-[10px] text-white/30 font-mono tracking-widest uppercase mb-3">¿Prefieres ver el reproductor funcionando ahora?</p>
          <button
            ref={demoRef}
            onClick={onLoadDemo}
